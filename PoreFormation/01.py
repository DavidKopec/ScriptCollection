...
# trajectory prep
def phase01() -> None:
    # assuming we are in the correct membrane directory
    os.system("mkdir FE")
    ...
    os.system(f"cp {PATH_SMD}/md0004.xtc ./{PATH_SMD}/storage")
...
def phase03_custom() ->None:
    ...
    for win_nr,frame in enumerate(frames_arr):
        ...
        time = float(data['time'][int(frame/norm_coef)])
        cv = float(data['cv'][int(frame/norm_coef)])
        
        temp = '%s\t%f\t%f\t%i\t%i' % (win_name,time,cv,int(round(frame)),win_nr)
        Out += temp+"\n"
        print(temp)
